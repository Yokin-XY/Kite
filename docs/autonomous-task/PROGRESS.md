...
| T003 | 已完成 | 系统来源只读 ID / 可编辑显示名称页面已实现 |
| T004 | 进行中 | 全量测试、构建、真机冷启动与收口 |
...
- 用户可见行为：免费/官方来源在模型库卡片提供编辑入口；受控页面逐模型显示可编辑“显示名称”和只读、可复制“模型 ID”。
- 保留行为：官方登录/退出动作仍由 `AgentOfficialAccountManager` 管理；自定义 Provider 仍进入原完整编辑页，可管理名称、URL、API Key 和模型 ID；系统来源无删除入口。
- 写入边界：系统来源保存只调用 `AgentModelLibraryStore.replaceProviderModelDisplayNames`，没有构造任何原生 Provider 配置变更。
- UI/压力：使用现有 `navigationHost` 的显式页面导航；扫描确认未新增 `showCardRunSurface`、资源刷新、轮询、文件/进程/网络探测、终端或 WebView 操作。新增 `navigationHost.removeAllViews()` 只发生在用户导航进入子页时，属于页面拓扑变化。
- 验证：模型库/设置相关目标测试通过，`BUILD SUCCESSFUL in 20s`，48 个 Gradle 任务（5 executed，43 up-to-date）；只有既有 `UiKit` deprecated 警告。

## 2026-08-01 T004 启动与三问自检

- 目标是什么：按 `PLAYBOOK.md` 的 T004，完成全量自动验证、Debug 构建、魅族 18 安装冷启动和崩溃/ANR检查，并整理稳定文档与交付证据。
- 完成标准是什么：测试/构建成功；目标设备安装冷启动成功且无本次崩溃/ANR；仓库不提交产物、日志、截图、设备状态或密钥；列出用户细致视觉与多步点击验收项。
- 依赖是否满足：T003 已通过目标测试和压力扫描，依赖满足。
