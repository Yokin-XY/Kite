...
- **新决定**:撤销原 ADR-017/018 的降级。T7-T9、T11、T4.2-4.4 全部继续做完。
- **执行策略(渐进,不搬运整套状态机)**:
  重度交互 Screen 用"Fragment 作为路由壳 + 内部复用 Activity 既有渲染方法"的渐进策略 ——
  先把 Screen 装进 Fragment(让 ScreenRouter 走 Fragment 路径),Fragment 内通过宿主接口
  调用 Activity 已经验证的渲染/状态方法,而非一次性把几百行状态机搬进 Fragment。
  这样每步都编译+测试+真机可验证,且不破坏已合规的 Store→信号→局部更新链路。
  后续可逐步把渲染逻辑内化进 Fragment。
