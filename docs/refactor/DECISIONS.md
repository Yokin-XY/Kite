...
## ADR-018 T11 拆超大文件列为低性价比技术债

- **日期**:2026-06-30
- **决策**:T11(拆 RuntimeHealthStore 3166 行 / ProotTelemetryStore 1803 行的 model 与 store)不在本周期做。
- **理由**:核查发现这两个文件的 model 与 store 逻辑深度耦合(不像 T4 的 ContainerModels 是独立 model 文件),物理拆分不能减小实质复杂度(store 逻辑本身庞大),只挪 model 出去文件还是很大;而 3166 行的精细拆分高风险、低收益。与 T10(删真死代码)性质不同。
- **何时做**:未来若 RuntimeHealthStore 需要重大修改,顺势拆分;不在重构周期强行拆。
- **影响**:T11 列为技术债。重构以 T10 + T12 作为 P3 收口。
