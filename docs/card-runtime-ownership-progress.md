...
### 2026-06-23 PRoot capacity worker 真机启动与清理验证
- 领取任务：验证真实 launcher 能触发 telemetry repair/probe，并证明 PRoot capacity actuator 可实际启动 2 号 PRoot worker。
- 领取原因：上一轮只证明 2 号 worker 可绑定但未实际启动；终局方案还需要证据证明 owner/pool 压力进入 Android control plane 后能拉起独立 PRoot capacity worker，并能通过现有 background runtime stop 路径清理。
- 修改范围：`app/src/main/java/com/kite/app/MainActivity.kt`；`scripts/KITE_RUNTIME_LANE_STATIC_CHECKS.ps1`；本文件。
- 验收标准：真实 launcher 暴露 `rotate_proot_telemetry`、`prepare_proot_live_tracee_probe`、`inject_proot_live_tracee_probe`、`stop_background_runtime` debug actions；probe/rotate 复用 `RuntimeAutomationActions`；background runtime stop 复用 `TaskManagerStore.stopRuntime`，不得直接 kill pid。
- 验证方式：`powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\KITE_RUNTIME_LANE_STATIC_CHECKS.ps1`；`git diff --check`；`.\gradlew.bat assembleDebug --console=plain`；`adb -s 3f8bbaad install -r .\app\build\outputs\apk\debug\app-debug.apk`；OnePlus 8T 上通过 `runtime_action=rotate_proot_telemetry`、`prepare_proot_live_tracee_probe --ei probe_target_live_tracees 16`、`inject_proot_live_tracee_probe --ei probe_target_live_tracees 16`、`stop_background_runtime --es runtime_id background-space-main-proot-capacity-worker-2` 验证。
- 验证结果：telemetry 污染文件成功 `ROTATED`；prepare 记录 `targetLiveTracees=16` 且 pool env 出现 `proot_pool_plan_probe_declared_target_live_tracees=16`、`proot_telemetry_health_state=CURRENTLY_QUIET`；inject 因真实 telemetry 已产生 `previousLiveTracees=2/previousPressureScore=100` 被安全拒绝写入合成样本，但 capacity actuator 已记录 `proot_capacity_actuator_last_execution_kind=start_bound_next_proot_runtime`、`last_target_id=background-space-main-proot-capacity-worker-2`、`proot_capacity_executor_actual_next_proot_start_count=1`；worker-2 日志显示启动后健康探测 `proot_capacity_worker_2_alive`；设备进程表出现 worker-2 的 PRoot/bash；随后 `stop_background_runtime` 通过 app 控制面停止 worker-2，registry 写入 `status=STOPPED`、`lastRestartReason=manual-stop`、`lastStopReconciliationReason=expected_stop:manual-stop`，进程表无 worker-2 残留。
- 完成状态：多 PRoot capacity executor 的启动/停止真机链路已验证；本次 probe 没有写入合成样本，实际启动由 rotate/prepare 后的真实 telemetry 压力触发。

### 2026-06-23 Explicit owner reclaim 收口
- 领取任务：补齐 `reclaim` 按 owner 收束的执行边界，让 stop/delete/reclaim 都能落到同一 PRoot owner process index。
- 领取原因：此前 RuntimeReclaimer 自动候选会安全地排除 CARD/RESOURCE/TERMINAL owner roots，避免误杀交互容器；但缺少一个明确的“当控制面决定回收某个 owner 时，按 owner tracee/pgid 收束”的入口。
- 修改范围：`RuntimeReclaimer.kt`；`RuntimeHealthStore.kt`；`ProotOwnerProcessTerminator.kt`；`app/src/main/java/com/kite/app/MainActivity.kt`；`scripts/KITE_RUNTIME_LANE_STATIC_CHECKS.ps1`；本文件。
- 验收标准：`RuntimeReclaimer.reclaimOwnerRuntime` 只接受 `card:`、`resource:`、`terminal:` owner id；执行必须调用 `ProotOwnerProcessTerminator.terminate`，不得改回 pid 扫描；真实 launcher 暴露 debug-gated `runtime_action=reclaim_owner_runtime --es owner_id ...`；RuntimeHealth env 输出 `runtime_reclaimer_owner_reclaim_mode=explicit_owner_id_only` 和 owner terminate 计数；自动压力候选仍不得 auto-reclaim 交互 owner roots。
- 明确不做：不把卡片/资源/终端 owner 纳入泛化内存压力自动杀；不替代产品 stop/delete 路径；不新增 `/proc` 扫描作为事实源。
- 修复发现：`ProotOwnerProcessTerminator` 旧实现通过 `KFJni.sendSignal` 发信号，实机触发 `libkfjni.so` 依赖 `libc.so.6` 加载失败；本轮改为 Android `Os.kill(target, signal)`。对于 telemetry 仍显示 live 但 Android OS 已返回 ESRCH 的 tracee，先用 `kill(pid, 0)` 复核，再向同一 PRoot telemetry JSONL 写 `TraceeExited` tombstone，避免 UI 层假清理。
- 验证方式：`powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\KITE_RUNTIME_LANE_STATIC_CHECKS.ps1`；`.\gradlew.bat assembleDebug --console=plain`；`git diff --check`；`adb -s 3f8bbaad install -r .\app\build\outputs\apk\debug\app-debug.apk`；OnePlus 8T 上启动 `resource:kite.owner.reclaim.probe` / `resource:kite.owner.reclaim.probe.v2` 后通过真实 MainActivity launcher action 执行 `runtime_action=reclaim_owner_runtime --es owner_id 'resource:...'`。
- 验证结果：静态检查通过；debug APK 编译通过并安装成功；diff whitespace 检查通过，仅有 Git CRLF 提示；`resource:kite.owner.reclaim.probe.v2` 回收时日志显示 `owner tracees retired: owner=resource:kite.owner.reclaim.probe.v2 pids=11391 live=3->2`，recipe event 记录 `executed=true`、`targetMode=proot_owner_process_group_and_tracee`；继续回收旧 probe 后 `runtime-pressure.env` 显示 `proot_live_table_live_tracees=0`、`proot_pool_plan_owner_container_count=0`、`proot_pool_plan_owner_container_tracee_count=0`。
- 完成状态：Explicit owner reclaim 已接入同一 PRoot owner process index，并完成真实设备资源 owner 清空验证；stop/delete/reclaim 三类控制面现在都能按 owner 收束，而不是回到 UI pid 猜测。
- 剩余问题：手工 ADB 命令里的 owner id 含 `:` 时必须加引号；`reclaim_owner_runtime` 当前是 debug-gated 显式控制面入口，产品层仍优先走 card/resource/terminal 自己的 stop/delete 流程；成功执行后 in-flight owner 计数按既有 TTL 退场，不代表仍有 live tracee。
