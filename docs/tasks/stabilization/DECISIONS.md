...
## ADR-S057 首页卡片通知区分运行事实与低成本结果

状态：accepted

决策：首页卡片运行中通知是不可清除的实例控制面，折叠态只显示当前主动作，展开态显示当前
允许的完整动作。非最终人工步骤的主动作是精确“下一步”；最终人工步骤和没有继续动作的运行态
主动作是“关闭”，并复用既有停止编排。终态默认撤销；卡片明确开启保留后，才投影为可清除、
不占运行资源的结果通知，并通过正式配方动作链创建新运行代次。

理由：最终步骤不存在下一步骤，继续命令会把“结束当前实例”和“完成当前步骤”混为一谈。
终态结果是否值得长期保留取决于卡片用途，不属于全局通知权限；结果通知也不能反向持有终端、
进程、WebView 或运行租约。

影响：保留策略写入卡片 `launch` 配置；`CardRunStore` 继续是唯一运行事实。用户清除结果通知只在
Platform 层记录 `instanceId + generation` 的展示抑制，新代次自然失效。资源安装通知以后可以复用
视觉壳，但必须由 `KiteResourceInstallStore` 单独投影，不进入本任务状态机。

兼容策略：运行通知使用版本化渠道 `kite_home_run_progress_v2`，并删除废弃的 `kite_run_progress`。
Android 不允许应用原地提高既有通知渠道的重要级，继续复用旧 ID 会让升级用户永久停留在默认级别。
新渠道声明高重要级并交由系统决定横幅；Kite 不增加应用内假弹窗，也不绕过用户的通知设置。

## ADR-S056 “下一步”由精确步骤通知承载

状态：accepted

决策：运行步骤的人工确认不再放在实例总览、报告页、终端页或 Web 页。进程级通知从
`CardRunStore` 的唯一事实投影实例进度；只有当前可继续步骤显示“下一步”。通知动作必须携带
`instanceId + generation + stepIndex + stepId`，由 `RunOrchestrator` 校验后才能推进。实例总览只
保留停止、加号和返回；竖条只管理显示面工具栏与实例总览，不承担步骤动作。

理由：继续动作属于产生它的步骤，而不是实例或页面的通用控制。只传实例 ID 会让旧页面、迟到
按钮或后续步骤误完成当前步骤；把按钮塞进每个显示面又会与终端、Web、SH 各自工具栏冲突。
通知天然同时承载进度、结果和离散动作，并且不依赖某个 Activity 仍然可见。

影响：运行事实仍由 `CardRunStore` 持有，通知只做投影和短期动作反馈，不得新增第二运行 Store。
通知不可用时，Shell 只能引导恢复通知能力，不能另加应用内“继续”兜底。Web 工具栏、终端工具和
SH 工具分别由各显示面拥有，本任务只给 Web 的隐藏/显示补动画。通知能力检查必须发生在资源、
配方和安装队列写入处理中事实之前；权限引导由共享无界面宿主持有，Activity 不保存第二份权限流程状态。
...
## ADR-S055 T012 以模块化单体封口，不为目录形状拆 Gradle 模块

状态：accepted

决策：稳定化完成后继续使用 `:app` 模块化单体，并保留已有的 `:terminal-view-local` 独立渲染模块。Shell、Feature、Application、Domain 和 Platform 的依赖方向由包合同、组合根和静态守卫执行。只有某个模块已经不穿越旧根包/`foundation` 依赖，并且独立构建、发布或替换能产生实际收益时，才单独提出 Gradle 模块拆分任务。

理由：T001-T012 的目标是转移状态、动作、页面和生命周期所有权，现有用户行为已经在同一 APK 内完成闭环。此时按目录机械拆模块不会减少运行耦合，反而会把过渡依赖固化成公共 API，并扩大构建配置和迁移回归范围。

影响：`MainActivity` 的职责清空、Feature 隔离和 Platform 边界继续以行为合同判断，不以 Gradle 模块数量判断。后续体积、启动或可替换组件若提供了真实拆分收益，应建立独立目标和验收基线，不回到本轮做无边界扩张。

## ADR-S056 通知能力遵循 Android 频道标准，不做厂商特判

状态：accepted

决策：Kite 把通知能力分为应用通知权限、用途明确的通知频道和具体通知实例。首页卡片使用独立高重要级
频道；容器前台服务使用静默低重要级频道；设置与首次启动引导直接进入首页卡片具体频道。应用只判断
通知是否可投递，不猜测或复制厂商系统中的横幅、锁屏等私有显示状态，也不按华为、小米、OPPO、vivo
或机型增加分支。

理由：Android 8.0 起由系统提供通知频道，Android 13 起增加 `POST_NOTIFICATIONS` 运行时权限；频道
重要级决定通知是否具备横幅资格，但最终展示仍由系统和用户设置控制。各厂商可以调整设置页面和增加
显示选项，却仍消费同一 Android 频道。应用内伪开关既不能改变系统事实，也会让权限问题难以诊断。
官方依据：Android 通知权限、通知频道与前台服务文档：
...
影响：无调用的 `FlaskApiService` 删除；仍承担容器和终端入口的 `KFShellService` 不能移除前台通知，
只能改为非打扰的后台运行频道。旧安装通过引导版本迁移只进入一次卡片频道设置。当前 targetSdk 仍为
28，Android 13 设备上的权限弹出时机继续由系统兼容策略控制；提升 targetSdk 涉及更广行为变更，应另立
任务验证，不能夹在本次通知修复中。

## ADR-S057 流程窗口可重置，手动窗口可独立关闭

状态：accepted

决策：实例总览中的固定流程窗口由配方步骤拥有，卡片左侧提供“重新执行”，不提供独立关闭；用户通过
“新建”创建的终端和网页是实例的独立子窗口，卡片右侧提供关闭，不提供流程重置。新建终端必须分配独立
会话，关闭只回收该子窗口；停止整个实例时统一回收全部子窗口。

理由：流程窗口是任务步骤的可见投影，单独删除会让配方事实和窗口拓扑分叉；手动窗口不属于固定步骤，
强行保留到实例结束又不符合桌面窗口习惯。左右分置还能避免用户把清空终端历史的重置误认为普通关闭。

影响：当前步骤重置通过 `RunOrchestrator` 清理旧执行后重放同一步，迟到回调必须由执行尝试身份拒绝；
历史步骤重放使用独立子运行，不推进父流程。窗口选择以 `windowId` 为身份，不能再只按 Surface 类型去重，
也不能让新终端复用父实例已有的 `TerminalFragment` 或 session。

## ADR-S058 实例容器以显式 owner 拓扑管理，不新增隔离层

状态：accepted

决策：Kite 的实例容器是运行所有权与回收边界，不是新的 Linux namespace、容器或 PRoot 隔离层。
每个实例使用 `instanceId + generation` 标识代次；流程步骤、手动终端、手动网页、SH 与资源操作使用
唯一叶子 owner。PRoot 遥测以 session、PRoot 启动代次、宿主 PID 和 tracee PID 复合标识进程生命周期。
实例关闭递归收集全部叶子 owner，并在遥测来源健康的前提下执行发现、关停和复核；缺失或不完整只能
返回 Unknown，不能由空集合推导成功。运行管理消费同一显式拓扑，不再通过评分猜测归属。

理由：原生 owner 遥测已经提供足够的派生和终态事实，重新增加隔离层会扩大体积和兼容风险；真正缺失的
是可靠身份、完整性门和递归回收。裸 PID、按资源 ID 共享 owner、手动终端无 owner 以及一次性首批 kill
都会让扫描结果与真实回收结果分叉。把这些问题在状态拥有者和执行边界收口，才能证明实例停止覆盖子孙。

影响：页面离开继续不等于停止实例；共享后台服务必须使用租约或引用计数，不能挂到任一实例叶子下。
墓碑只能在直接存活性复核后写入。后续运行管理 UI 只做状态投影和提交动作，不负责扫描、猜测或宣布成功。
当前 `BackgroundRuntimeHost/linkedRuntimeId` 保持独立运行根并沿用自己的生命周期租约，不进入实例拓扑；
只有未来出现“实例显式获取共享服务”的生产路径时，才增加成对 acquire/release 引用，禁止预先放置无人消费的
平行计数状态。

## ADR-S059 遥测历史裁剪采用 owner 代次覆盖边界，不永久封锁新实例

状态：accepted

决策：PRoot 遥测读取器可以为追上当前状态而跳过旧 JSONL 积压，但必须记录 owner 证据从哪个时间点开始连续
完整。破坏性停止按 owner 中的实例 generation 与该覆盖边界比较：边界之后的新实例可以进入发现、TERM/KILL
和直接探测；边界之前、代次不可解析或覆盖边界未知的 owner 继续返回 Unknown。`skippedBytes` 只保留为历史
裁剪诊断，解析错误仍是全局完整性阻断项。

理由：大日志首次只读取末尾 256 KiB 是明确的追赶策略，不代表此后写入的新实例事件损坏。把累计
`skippedBytes` 当成永久健康门，会让一次旧历史裁剪封死本进程后续所有实例；直接忽略裁剪又会错误放行证据不全
的旧实例。时间覆盖边界与结构化 owner generation 已经能表达这两类事实，不需要设备特判或盲目强杀。

影响：遥测健康、压力观测和池策略不再因历史裁剪永久降级；owner 停止仍必须满足来源新鲜、无解析错误、代次在
覆盖区间内、owner 已观察和 PID/PGID 直接探测为空。底层 `__kite_owner_stop_*` 标记只进入 Bridge 判断、诊断与
报告，用户界面只显示收敛后的中文结果；任何非 `CONFIRMED` owner 都不能由空残留列表冒充停止成功。

## ADR-S060 用户停止与旧代进程回收分离，实例状态只允许单向闭合

状态：accepted

决策：用户停止的可见状态机固定为 `Running -> Stopping -> Stopped`。终端、子窗口、root/leaf owner、PID、
URL 和当前运行绑定随逻辑实例一次性清除，不再根据同一时刻的残留探针恢复旧状态。`OWNER_NOT_FOUND` 表示本次
目标已经不存在，可直接收敛；明确 PID/PGID 残留、探针超时或遥测暂不可用仍保留为执行诊断，并由代次化 owner
后台去重重试回收，但不得阻塞同一实例的新代次。运行管理只等待被捕获的旧代 CardRun 子树闭合，不等待旧代
进程和终端扫描结果归零。

理由：真机证据显示最终 owner 探针比终端真实 SIGTERM 早约 62ms，探针返回残留后所有 PID 随即消失。把这种
瞬时残留解释成 `Restore(previousState)`，会把已经停止的卡片、终端和 owner 绑定整份复活，并让旧命令占用
`run:<instanceId>` 阻塞下一次启动。用户动作的确定结果与后台操作系统收敛速度是两类事实；只有分离二者，才能
同时保证停止确定性和底层诊断真实性。

影响：ADR-S023、ADR-S058 和 ADR-S059 中“非 CONFIRMED 必须恢复或保持当前运行事实”的部分由本决策取代；
底层仍不得伪造 PID 已退出，也不得盲目扩大 kill 范围。后台回收只使用旧代精确 owner，最多进行有限重试；新代
owner、共享服务和独立终端不进入旧代回收。停止成功不弹 Toast，页面只从同一 `CardRunStore` 投影回“启动”。

## ADR-S061 Bridge 停止接口必须异步且串行执行阻塞工作

状态：accepted

决策：`KiteBridgeClient` 的 direct run、direct process 和无直接绑定停止共用单线程后台队列。调用线程只能
提交停止任务，不能直接执行遥测重读、owner 发现、shell 探测、TERM/KILL、`Process.waitFor` 或稳定静默确认。
停止结果继续通过原 callback 回写既有状态拥有者，不新增轮询、页面本地状态或第二套停止事实。

理由：真机两次输入 ANR 的主线程堆栈都落在无直接绑定停止分支：一次睡眠于 Ubuntu shell 的
`Process.waitFor`，一次持续解析 `ProotTelemetryStore.refreshBlocking`。该分支虽然暴露 callback，却同步调用
重型实现，违反接口语义；direct 两条路径原本各建线程，也允许多个停止并发争抢同一遥测锁。统一串行队列同时
修复线程边界和遥测互斥压力。

影响：首页点击仍立即进入 `Stopping`，后台完成后按 ADR-S060 单向闭合为 `Stopped`。停止期间导航、返回和
其他页面输入保持可响应；多个实例同时停止时会排队执行精确 owner 回收，而不是在 UI 线程并行扫描。不得通过
缩短探针、跳过 TERM/KILL 或删除进程复核来换取响应速度。

## ADR-S062 正式文档与任务流水分层，实验实现保留但退出稳定承诺

状态：accepted

决策：Git 跟踪分为必须跟踪、按复用价值跟踪和不得跟踪三类。长期有效的产品事实进入 `README.md`、
`docs/architecture/`、`docs/guides/` 和 `docs/reference/`；跨会话任务三件套只在任务期间进入
`docs/tasks/<task>/`。一次性截图、日志、诊断、APK 和临时报告进入 `local-artifacts/` 或发布系统，不能作为
正式文档长期保留。X11 与浏览器自动化代码本轮保留，入口和资源明确标为实验能力，稳定版本不依赖也不承诺。

理由：原先整体忽略 `docs/` 再逐目录放行，使长期协议没有版本控制，而已完成任务的数千行进度和真机证据反而
长期留在主线；README 与真实模块和运行链逐渐分叉。直接删除实验代码会扩大风险，继续无提示展示又会混淆发布
边界。按用途分层可以让源码、正式说明、当前任务和本地产物各自拥有明确生命周期。

影响：任务完成时必须先把仍有效决定提炼进正式文档，再删除流水；新增正式文档不得复制任务时间线。浏览器
自动化和 X11 可以继续专项开发，但不得写入稳定能力完成清单。`.gitignore` 不再屏蔽 `docs/`，文件退出跟踪时
必须同时清理本地内容，不能用新增忽略规则掩盖历史残留。

## ADR-S063 工作负载作用域由 PRoot 生命周期事实自动生成，名称只用于展示

状态：accepted

决策：在外层 owner 与单个进程生命周期之间增加 `workloadScopeId`。该身份由 PRoot 遥测中的强生命周期、
父子派生关系和启动边界自动生成并由子孙继承，不由资源清单、卡片配方或应用名称提供。运行管理所有页面统一
按该身份组织应用/任务组；没有 owner 的作用域显示为未归属，但仍保留自己的成员树。显示名称、命令名、
可执行文件名只决定文案，不能参与身份相等或破坏性动作范围。

理由：Linux 没有可供普通应用直接读取的通用“桌面应用 ID”，同名可执行文件可以属于不同任务，单个应用也
可能派生多个不同名称的进程。Windows 任务管理器的应用组同样依赖平台/启动器提供的运行边界，而不是仅按
名称拼接。Kite 已经控制 PRoot 启动与派生事实，系统生成作用域比维护微信、Python 等应用表更通用，也能让
未来 cgroup 后端直接承接同一合同。

影响：`ownerId` 继续承担卡片、资源、终端等外层生命周期；`lifecycleId` 继续标识单个进程代次；
`workloadScopeId` 只承担一次工作负载的归组和整组动作。整组结束必须从作用域索引枚举并复核成员，不能把
共享 PGID、SID、显示名称或当前 UI 列表当成隔离边界。当前无 cgroup 写权限的设备使用 PRoot 遥测后端；未来
增加 `CgroupBackend` 时只替换成员登记、探测和终止实现，不改变页面协议。破坏性动作只要求目标作用域已知
会话的活动注册表完整；全局其他历史会话为 partial 不能扩大范围，也不能阻塞目标会话的精确停止。
