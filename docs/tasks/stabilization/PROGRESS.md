...
方向：主线边界与文档治理
状态：completed
当前任务：T019 已完成；未自动创建后续任务
代码分支：main
代码策略：正式文档与任务基础设施已分层；X11 与浏览器自动化保留实现并明确标为实验能力
```
...
| T019 主线边界与文档治理 | completed | 三类跟踪边界、正式文档、实验标识、历史清理及全量验证均已完成 |
...
### 2026-07-14 T013 实例窗口交互与通知信号封口

三问自检：

- 目标：让“下一步”成为绑定精确运行代次和步骤的通知动作，并恢复成熟的实例窗口操作模型。
- 验收：错误步骤不能推进；每实例通知及时投影；实例总览恢复三按钮与加号二选一；Web 工具栏平滑展开；终端/SH 本体不变。
- 前置：T006 运行编排、T007 独立运行壳、T012 发布验收均已完成；当前未提交的浏览器认证改动不属于本任务，不触碰。

已确认产品规则：

- “下一步”不属于实例全局动作，也不属于终端、Web 或 SH 固定工具栏；通知是唯一操作入口。
- 动作必须携带 `instanceId + generation + stepIndex + stepId`，由 `RunOrchestrator` 对当前事实逐项校验。
- 实例总览底栏为停止、加号、返回；加号动画展开网页和终端入口。
- 竖条单击只切换当前显示面自有工具栏；双击打开实例总览；长按拖动位置。
- 本轮不修改终端输入栏、终端更多菜单或 SH 报告工具本体。

实现与验收结果：

- `RunStepCompletionCommand` 固定提交 `instanceId + generation + stepIndex + stepId`；
  `RunOrchestrator` 对当前运行事实逐项校验，旧代次、错误序号和错误步骤 ID 均由单测证明不会推进。
- 进程级通知协调器按实例投影步骤、进度、等待、完成、失败和停止结果；只有共享步骤策略认定可继续时
  才携带精确身份显示“下一步”。停止 OpenCode 后，真机通知立即由等待态变为“终端已发送中断并关闭”。
- 通知能力成为统一运行启动门。资源、配方、安装队列和独立运行窗口都在写处理中事实前检查；权限不足时
  只显示授权说明和等待页，不创建新实例、不启动终端、不推进安装队列。权限宿主位于无界面 Fragment，
  MainActivity 没有新增权限状态副本。
- 实例总览真机恢复停止、新建、返回三项；新建以动画展开终端和网页两个圆形入口。竖条双击打开总览，
  单击以胶囊动画隐藏/恢复 Web 自有工具栏；终端输入栏和 SH 工具本体未修改。
- 覆盖安装时首次看到的是设备中尚未消费的旧 `TerminalFragment id=0x2` 真实崩溃报告；点击一次重试后，
  连续冷启动均直接进入 MainActivity，证明诊断记录按既有规则消费，没有产生新的启动拦截或崩溃。
- 全量 `:app:testDebugUnitTest` 为 465 项、0 失败、1 项既有跳过；架构和运行车道守卫通过，
  MainActivity 棘轮为 `2577 / 126 / 41`（行 / 函数 / 私有字段），低于 T012 基线。
- 多轮增量编译曾让 APK 因残留 DEX 虚增至 245,924,705 bytes；干净构建后体积为 231,268,249 bytes，
  assets 压缩体积 183,990,598 bytes，体积守卫通过，SHA-256 为
  `8203cbd3848516f80c217ff88b33234bd7eb96865ccbaa1bdd17fbc6d7d49182`。
- 最终干净 APK 已覆盖安装到 OnePlus 8T；冷启动 1290ms，主界面、实例总览、终端、Web、通知和权限门
  均有真机证据，logcat 未见 FATAL、ANR 或 `No view found for id`。

T013 状态：completed。本任务不包含 X11、完整浏览器或终端工具栏扩张。

### 2026-07-14 T014 首页卡片通知生命周期封口

三问自检：

- 目标：只完善首页卡片运行通知，让折叠、展开和横幅都按同一状态模型表达关闭、下一步和结果。
- 验收：最终步骤只能关闭；中间人工步骤可精确继续；卡片可选择保留可清除的终态通知并从正式动作链再次运行。
- 前置：T013 已完成；现有 `RunNotificationProjector`、`RunOrchestrator`、配方编辑器和通知接收器入口真实存在，可以增量收口，不需要新增运行 Store。

本轮边界：不实现资源卡片或安装向导通知；不修改终端、Web、SH 显示面；不为特定配方或设备写特判。

完成内容：

- 首页卡片 `launch` 增加 `keepFinishedNotification`，默认关闭；编辑器“更多配置”提供“保留结束通知”
  开关，JSON、草稿恢复、新建和编辑链路均使用同一字段。
- 通知投影只消费 `ownerKind=card` 的 `CardRunStore` 事实。中间人工步骤折叠态为“下一步”，展开态为
  “关闭/下一步”；最终步骤只保留“关闭”，不再伪造完成命令。
- “关闭”复用既有停止和实例关闭编排；终态默认撤销，明确保留时才转换为非持续、可滑动清除并提供
  “再次运行”或“重试”的结果通知。再次运行读取最新配方并通过正式动作网关创建新代次。
- 用户清除结果通知只持久化 `instanceId + generation` 的展示抑制；同代次在 Store 更新和进程冷启动后
  不复活，新代次自然恢复。广播接收使用 `goAsync()/finish()`，不依赖接收器短生命周期。
- 折叠、展开和系统横幅共用一份通知模型与自定义 `RemoteViews`。运行通知迁移到
  `kite_home_run_progress_v2` 高重要级渠道，并删除废弃的 `kite_run_progress`，解决 Android 渠道重要级
  创建后不能原地升级的问题。系统仍决定横幅弹出时机；同应用前台可能按厂商策略抑制重复横幅。

验证证据：

- 全量 `:app:testDebugUnitTest` 为 475 项、0 失败、1 项既有跳过；`assembleDebug`、架构守卫和运行车道
  静态守卫均通过，MainActivity 棘轮保持 `2577 / 126 / 41`。
- OnePlus 8T 覆盖安装成功。实测中间步骤折叠“下一步”、展开“关闭/下一步”，下一步只推进精确的
  当前代次和步骤；最终步骤只显示“关闭”。
- 开启保留后，关闭会生成可清除结果通知并提供“再次运行”；清除后冷启动不复活，重新从首页启动
  新代次后通知恢复。关闭保留后，同样流程关闭即撤销通知。
- 编辑器真机可见“保留结束通知”，默认关闭；旧渠道已删除，新渠道为 `IMPORTANCE_HIGH` 且设备报告
  `mShowBanner=true`。最终冷启动 1206ms，首页恢复原有 2 张卡，临时验收卡和通知均已清理，logcat
  未见 FATAL 或 ANR。
- 最终 Debug APK 为 232,619,400 bytes，SHA-256 为
  `7cc01a0d1039bd22fa9fa67356dcac8336fd535740349e0a883e934373a82013`。

T014 状态：completed。本任务没有接管资源安装、安装向导、独立终端或 Web 通知。

### 2026-07-14 T015 通知权限与服务频道收口

三问自检：

- 目标：让首页卡片通知在首次安装和升级后都能进入正确的系统权限链，同时移除无效服务通知。
- 验收：只引导一次；卡片频道可弹出；后台容器通知静默；设置页直接进入具体频道；无厂商特判。
- 前置：T014 已建立首页卡片通知模型和独立频道，现有首次启动协调器与设置 Feature 可以增量接入。

完成内容：

- 按 Android 标准拆分为应用通知权限、首页卡片频道和通知实例三层。`AndroidRunNotificationAccess`
  只判断是否可投递，并提供首页卡片具体频道与应用通知设置 Intent；横幅和锁屏仍由系统及用户决定。
- 首次启动引导版本升级为 2：新安装和已完成旧引导的升级安装都会在已有权限检查之后打开一次
  “首页卡片进度”频道；从设置返回后完成记账，后续冷启动不再重复。
- 设置页将原来的通知伪开关改为“首页卡片通知”导航项，展示真实授权状态并直接打开具体频道。
- 首页卡片继续使用 `kite_home_run_progress_v2` 高重要级频道；容器前台服务改用
  `kite_background_runtime_v2` 低重要级静默频道，不发声、不振动、不显示角标。
- 删除没有任何调用方的 `FlaskApiService` 及 Manifest 声明；升级时删除旧的 `kfshell_service`、
  `flask_api_service` 和 `kite_run_progress` 频道。KFShell 前台服务仍保留，因为 Android 要求运行中的
  前台服务必须显示通知。

验证证据：

- 全量 `:app:testDebugUnitTest assembleDebug` 为 480 项、0 失败、1 项既有跳过；架构守卫和运行车道
  守卫通过，MainActivity 棘轮保持 `2567 / 126 / 41`。新增频道 Intent、首次引导版本迁移和设置投影测试。
- OnePlus 8T 覆盖安装后首次冷启动直接进入系统“首页卡片进度”频道页面；返回后再次强停冷启动
  直接进入 `MainActivity`，进程存活且 logcat 未见 FATAL、ANR 或通知异常。
- 系统通知状态报告首页卡片频道 `importance=4`、`mShowBanner=true`；后台运行频道
  `importance=2`、`sound=null`、无振动、无角标、无横幅；两个旧服务频道均为 `mDeleted=true`。
- Kite 设置页真机显示“首页卡片通知”及当前授权说明，点击后进入同一具体频道。
- 通过产品入口新建终端后，`KFShellService` 只发布 `kite_background_runtime_v2` 前台服务通知，系统
  报告无声音、无振动且属于 `service`；最终强停重启后空闲态不运行该服务。最终冷启动 1309ms，
  logcat 未见 FATAL、ANR、`RemoteServiceException` 或 Bad notification。
- 最终 Debug APK 为 232,752,660 bytes，SHA-256 为
  `bd3f6a0759cae55635202bb5cbde1efcedce7f15fd718dc24260fc0911ce2da6`。

T015 状态：completed。本任务没有提升 targetSdk，也没有改动资源安装通知或厂商私有通知接口。

### 2026-07-15 T016 实例窗口重置与独立子窗口

- 固定流程窗口左侧提供重新执行，保持窗口存在；手动新建窗口右侧提供关闭，左右操作不复用位置，避免
  误把终端重置当作普通关闭。
- 当前步骤重新执行由 `RunOrchestrator` 先清理本步骤旧执行，再以新的 attempt 身份重放；迟到回调不能
  写入新尝试。历史步骤通过 replay 子运行重放，不推进父流程。
- 实例总览按稳定 `windowId` 投影，不再按终端、网页等 Surface 类型去重；同一实例可以拥有多个独立
  终端或网页窗口。
- “新建终端”创建独立子运行和独立 shell session，关闭只结束对应会话；停止整个实例会统一回收手动
  终端、手动网页和步骤重放子窗口。
- 当前 Debug APK 编译成功并覆盖安装到 OnePlus 8T。用户人工确认新建分支、独立终端、重置和关闭功能
  已达成，本轮不再执行自动交互测试。
...
### 2026-07-15 T018.4 显式实例运行拓扑

完成结果：

- 新增纯事实 `InstanceRuntimeTopology`，以 `instanceId + generation` 标识节点，并按真实
  `parentInstanceId` 递归形成根、子、孙层级。
- 进程归属只接受 Store 已保存的 root/leaf owner、终端 session、绑定 PID，以及已唯一归属父进程的真实
  `parentPid`；无法唯一确定时保留为未归属或歧义，不再用 50/40/30/20 分数猜测。
- 运行管理只展示拓扑根实例，子孙显示面和进程由同一子树投影；独立终端和未归属进程继续保留显式区域，
  不会因拒绝猜测而消失。
- 新增拓扑合同测试，覆盖精确 owner、真实父 PID 继承、三层实例递归、共享 owner 歧义拒绝和旧 owner 猜测
  拒绝；运行管理投影与 Android Gateway 目标测试通过，生产 Kotlin 编译通过。

T018.4 状态：completed。下一步让实例停止按该拓扑最深层优先关闭子孙，并等待每个关闭结果后再停止父实例。

### 2026-07-15 T018.5 实例子孙递归关闭与统一确认

完成结果：

- `RunOrchestrator.stop` 先把父实例写为 `Stopping`，再等待子窗口关闭结果；子窗口未确认时恢复父实例原状态并
  投影明确失败，底层父停止不会提前发出。
- `AndroidRunWindowSurfaceGateway` 从同一实例拓扑取出全部子孙，按最深层优先串行关闭；每个子运行只有得到
  `StopCoordinator` 的确认结果后才移除，失败或代次变化会保留事实并中止父停止。
- 子窗口和父实例关闭回调均携带 generation。旧子代次、旧父代次或启动后迟到的回调不能删除新事实，也不能
  停止新进程。
- 运行管理提交停止时捕获目标子树的实例代次、owner、终端与进程证据；根卡片变成 `Stopped` 或暂时消失都
  不足以宣布成功，旧子实例、owner 进程或活终端任一残留都会继续保持待确认。
- 共享后台运行项仍是独立 `linkedRuntimeId` 根，既不按 owner 分数归入 CardRun，也不进入 parent 子树；实例
  关闭不会触碰它。当前没有实例 acquire 共享服务的生产路径，未来新增时必须先建立成对租约，不新增空转计数。
- 目标测试覆盖父等待、子失败、旧代次拒绝、孙先于子、失败子事实保留、停止后进程残留、根缺失但子仍存在，
  以及共享后台运行项不归入实例子树；生产 Kotlin 与目标测试通过。
...
### 2026-07-15 T019 主线边界与文档治理

三问自检：

- 目标是什么：重建 Git 跟踪边界，并依据当前源码从零建立正式项目文档；旧任务流水和一次性证据不再冒充
  产品说明。X11 与浏览器自动化保留实现，但明确退出稳定版本承诺。
- 完成标准是什么：`docs/` 可以正常跟踪；正式文档按架构、指南和参考分类；长期任务进入 `docs/tasks/`；
  构建物、本机状态和一次性证据可靠忽略；旧文档、截图和续跑脚本退出；文档、护栏、单测和构建通过。
- 依赖是否满足：当前主线已有稳定卡片、资源、终端、运行管理和系统浏览器认证链，且 T012 已完成模块化单体
  封口；本轮只整理事实、文档和实验提示，不改稳定业务默认路径，依赖满足。

完成结果：

- `.gitignore` 取消 `docs` 白名单式管理，明确忽略构建物、密钥、本机配置、诊断和 `local-artifacts/`；
  `AGENTS.md` 固定“必须跟踪、按复用价值跟踪、不得跟踪”三类规则。
- 从当前源码重写 `README.md`、`CONTRIBUTING.md`、架构总览、状态与生命周期、长期决策、构建、卡片资源、
  浏览器认证、验证方式，以及首页卡片、资源清单、Bridge 和能力状态参考。
- 当前稳定化任务迁入 `docs/tasks/stabilization/`，架构检查基线引用同步迁移。旧浏览器分支、X11 分支、重构、
  状态链、资源安装任务流水，设备截图和 17 个定时续跑脚本退出工作树。
- 浏览器自动化设置入口继续显示“实验入口”；两个 X11 资源清单增加“实验能力”徽标和稳定承诺边界。
  设备导出的卡片说明改指向 `docs/reference/home-card-schema.md`。
- 修正运行车道护栏中 4 条重构前字符串断言，使其检查当前实例关闭、精确通知身份、窗口脱离/停止终态和
  `TaskManagerSnapshotGrace` 合同；架构与运行车道检查通过。
- Markdown 链接、旧路径引用、21 份资源 JSON 和 Git 忽略反查通过。全量
  `:app:testDebugUnitTest :app:assembleDebug` 通过：535 项测试、0 失败、0 错误、1 项既有跳过。
- Debug APK：`app/build/outputs/apk/debug/app-debug.apk`，232,316,380 bytes，SHA-256
