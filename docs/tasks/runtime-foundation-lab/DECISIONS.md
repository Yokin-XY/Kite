...
## ADR-RF-068 压缩默认执行集，不删除完整测试资产

- 状态：已接受，RF1550 已完成
- 日期：2026-08-01
- 决定：276 个测试类继续作为 Full 发布门；叶子任务默认使用 Targeted/Stage，日常跨层护栏使用不超过总类数 25% 的 Quick 命名族，父任务、合并和发布才执行 Full。测试入口统一输出实际 XML 摘要，不能只凭 Gradle exit code 宣称覆盖。
- 原因：当前 1465 tests 来自持续增加的有效合同，不是重复注册；直接删除会丢失过去修过的问题。真正浪费是每个叶子都用 `--rerun-tasks` 重跑全量，而没有稳定的中间层。
- 影响：历史全量证据仍有效；Quick/Stage 不能替代最终 Full。新测试命名应继续使用稳定职责后缀，使其能进入相应层，不建立第二份测试源码树。

## ADR-RF-069 本机 Kite Gradle 重任务默认串行协调
...
## ADR-RF-070 受管命令原生证明先补可执行事实再接生产

- 状态：已接受，RF1520 已完成
- 日期：2026-08-01
- 决定：资源打开与获取共享的首次 `managedCommands` 核对进入 RF1530；候选只能依据普通默认容器的完整运行时、固定 PATH、受控符号链接链、普通文件和执行位事实返回肯定结果。未知或不完整事实继续现有 PRoot Probe。
- 原因：OnePlus 8T 首轮 p50 从 PRoot 106ms 降至 Android 8.966ms，通用收益明确；但现有 `managedCommandVerificationBasis()` 没有执行位，固定无执行位文件被误判为存在。该假阳性说明文件“存在”不等于 shell 命令“可用”。
- 影响：RF1520 Debug 证据不改变生产行为。RF1530 必须先让四类固定事实连续三轮与 PRoot 一致，并满足预先固定的性能门；不允许按资源 ID、命令名或应用特判，也不迁移安装脚本中的任意 shell 检查。

## ADR-RF-071 完整肯定式受管命令证明可跳过首次 PRoot Probe

- 状态：已接受，RF1530 已完成
- 日期：2026-08-01
- 决定：普通默认容器中，运行时、安装记录、固定 PATH、受控符号链接链、普通文件和执行位事实全部完整时，`ResourceManagedCommandNativeProof` 可直接满足本次正向核对；同一批次中无法证明的资源仍交给既有 PRoot Probe。
- 原因：执行位反例修复后，四类固定事实连续三轮与 PRoot 完全一致；原生 p50 为 10.394～11.930ms，相对 PRoot 105～106ms 减少 88.6%～90.2%，原生 p95 均低于预设 30ms。首次 p95 超门后通过减少重复文件 stat 达标，没有事后放宽阈值。
- 影响：资源安装登记、缺失失效、页面状态和 Probe 都保留原拥有者；没有新 Store、命令/资源白名单或安装脚本迁移。RF1540 仍需用真实资源打开/获取链和 Full 门决定父任务最终 go/no-go。

## ADR-RF-072 受管命令原生证明进入生产

- 状态：已接受，RF1540 与 RF1500 已完成
- 日期：2026-08-01
- 决定：保留 RF1530 的原生肯定式证明作为正式默认环境快速路径；完整事实直接满足核对，任何未知、不完整或非默认环境事实继续由既有 PRoot Probe 处理。观测只记录总数、原生、缓存和回退数量，不记录资源 ID 或命令。
- 原因：OnePlus 8T 三轮固定矩阵正确性完全一致，p50 相对 PRoot 减少 88.6%～90.2%；真实 OpenClaw“打开”命中 `native=1/fallback=0` 并进入 Agent 显示面，最终 Full 1473 项零失败。打开和获取继续调用同一核对函数，无需第二套状态或资源特判。
- 影响：该收益只覆盖首次受管命令正向核对，不代表整个资源或 OpenClaw 启动缩短同一比例。动态 shell、View、自定义 PATH、安装脚本内部检查、版本输出、别名、函数和实际命令执行均不在开放范围，继续 PRoot；后续通用优化从兼容债务总账另立父任务。

## ADR-RF-073 下一通用候选选择受管包元数据版本探针

- 状态：已接受，RF1610 已完成，RF1600 进行中
- 日期：2026-08-01
- 决定：RF1600 只研究默认 npm 来源的结构化已安装版本元数据读取。调用方必须显式提交受控容器路径、最大字节数和 JSON 字段；Provider 不读取资源 ID、包名、命令名或页面。普通受控文件中的字符串字段可形成原生 Ready；路径逃逸等无效请求 Blocked；符号链接、超限、格式/字段未知和旧自定义命令在首个业务进程前 Unsupported 并保留现有 PRoot。生产样板必须等 RF1620 固定正确性与收益门全部通过。
- 原因：正式目录有 5 个资源共享 `defaultInstalledVersionProbe()`，单项和批量检查更新当前为每个资源启动一次 PRoot + Node；Android 已掌握工作区物理映射和只读文件事实，候选覆盖真实高频链且不需要启动解释器。显式 `命令 --version` 仍缺少可执行文件、Linux ELF、子进程和环境闭包证明；取消清理包含活动事务和资源分支，均不满足本轮通用低风险门。
- 影响：RF1620 固定矩阵不接受 ADB 自定义资源、包、路径、字段、轮数或阈值；三轮必须零差异，原生 p50 每轮至少降低 70%、p95 不高于 30ms，固定 5 请求批次 p50 至少降低 60%。任一门失败即 no-go。RF1600 不修改远端版本请求、显式版本命令、安装事务、取消清理、Store、UI、PRoot View、Node/Python 或 RF1500 路径。
...
## ADR-RF-077 下一候选选择批量版本检查分车道有界并发

- 状态：已接受，RF1710 已完成，RF1700 进行中
- 日期：2026-08-01
- 决定：RF1700 只研究资源批量检查更新的调度等待。批量入口必须先为全部目标写入既有检查中状态，再让结构化原生元数据＋远端请求使用最多 3 个并发槽，让无结构化事实、需要显式命令/PRoot 的请求使用最多 1 个兼容槽；结果仍按输入顺序应用。调度依据只能是版本计划合同，不能解析命令或读取资源、包、页面身份。
- 原因：当前正式入口对所有目标使用同步 `map`，多个互不依赖的本地/远端版本请求完全串行。5 个默认 npm 资源已满足原生本地版本和独立远端请求，批量等待成为 RF1600 后下一处真实用户成本。显式版本迁移缺少 CLI/包版本等价证明；Ubuntu 小探针缺少 rootfs 语义闭包，均不应抢先。
- 影响：RF1720 必须先过固定五请求的顺序、并发、取消、失败和性能门，失败即 no-go。单项检查、版本解析、远端协议、安装/更新事务、Store、UI、PRoot View、Node/Python 和 RF1500/RF1600 均不改。用户明确说明正在改造的 AI 会话能力全面冻结，不进入本父任务的审计、补丁或性能结论。

## ADR-RF-078 固定矩阵通过后允许最小生产调度样板

- 状态：已接受，RF1720 已完成，RF1730 进行中
- 日期：2026-08-01
- 决定：允许把已证明的 `3` 个结构化原生/远端槽与 `1` 个兼容槽合同提升为 RF1730 最小生产样板。生产分类只能读取 `KiteResourceVersionCheckPlan` 的结构化元数据与远端探针类型；不满足完整合同的目标整项进入兼容槽，不能解析命令或按资源身份选择。
- 原因：OnePlus 固定五请求三轮零差异，顺序、检查中先写、失败隔离、取消清理和并发上限均通过；串行 913/911/910ms 降到 380/370/372ms，每轮减少 58.4%～59.4%，候选 p95 380ms，超过预设发布门。Targeted、Quick、Stage 与 Debug 构建均通过，当前日志无 FATAL/ANR。
- 影响：RF1730 只替换批量入口的等待调度，单项检查和每目标版本事实不变。Debug 候选在生产样板落地后必须改为复用生产调度器，防止平行实现；RF1700 最终 go 仍需 OnePlus 真实批量链和父门唯一 Full。AI 会话继续冻结。

## ADR-RF-079 批量车道由进程前物理预检关闭

- 状态：已接受，RF1730 已完成，RF1740 进行中
- 日期：2026-08-01
- 决定：生产批量入口不能仅凭“声明了结构化元数据”就进入 3 槽，必须在任何版本命令前实际运行受控 JSON Provider 预检。Ready 才缓存本地版本并进入结构化原生/远端槽；Unsupported 整项进入单槽兼容路线；Blocked 直接产出既有失败事实。调度器只消费已经关闭的 lane，不自行读取业务合同。
- 原因：结构化声明不等于文件、权限、大小和 JSON 字段在当前设备上 Ready。若先放进 3 槽再在内部回退，最多会同时启动 3 个 PRoot，破坏兼容车道上限。预检本身不创建业务进程或网络副作用，RF1600 Provider 已有严格 Ready/Unsupported/Blocked 语义。
- 影响：批量 Ready 路径不重复读取本地元数据，只并发独立远端请求；未知事实仍走原 `ResourceVersionCoordinator.check()`，但整项受兼容单槽约束。固定矩阵已委托生产实现并继续通过，单项检查仍沿用原路径。最终是否 production go 由 RF1740 真实批量链和父门 Full 决定，AI 会话继续冻结。

## ADR-RF-080 RF1700 批量版本检查生产放行

- 状态：已接受，RF1700/RF1740 已完成，production go
- 日期：2026-08-01
- 决定：保留资源批量检查的进程前物理预检与 `3/1` 有界调度作为生产行为。结构化元数据 Provider Ready 且最新版本为远端探针的目标进入最多 3 槽；事实不足或显式命令目标整项进入最多 1 槽兼容路线；Blocked 不启动命令。结果按输入顺序写回既有 Store，单项检查不变。
- 原因：固定五请求在生产调度器上三轮零差异，串行约 909～910ms 降为 371～377ms，最小减少 58.5%、p95 377ms；OnePlus 真实 2 个已安装正式目标命中原生/远端 1、兼容 1，去重为 2、检查中残留 0。最终 Full 1497 tests 零失败，当前进程无 FATAL/ANR。
- 影响：收益只覆盖资源管理页一次检查多个已安装资源时的累计等待，不等于远端响应、单项检查或应用启动统一减少同一比例。调度器、Provider、Store、页面和 PRoot 的所有权边界不变；下一候选重新从债务总账立项。AI 会话、魅族、main/其他工作树、远端、版本与发布均未触碰。

## ADR-RF-081 下一候选选择首次内置底层资源依赖调度

- 状态：已接受，RF1810 已完成，RF1800 进行中
- 日期：2026-08-01
- 决定：RF1800 只研究首次底层环境准备的 6 个正式内置资源。调度输入必须是任务键、显式依赖集合和待执行工作；最多 2 个活动任务，前置失败时后继不启动，独立任务继续，结果按输入顺序归并。不得解析安装脚本或按资源、包、命令、页面身份决定槽位。
- 原因：生产 `runPreparePackLocked()` 当前完全串行；OnePlus 现有成功记录为 44,411ms。6 项分别写入独立资源根，正式关系只声明系统工具依赖 Node.js，具备在首个安装进程前封闭调度的物理事实。资源清单重载只有 17 份/68,450 字节且已有缓存；正式资源内部多条安装子步骤已合并为一个事务 shell，两者都不应抢先伪装成高收益。
- 影响：RF1820 必须先用固定依赖反例和 APK 内置真实包通过三轮零差异、最大活动数 2、依赖/失败/清理门，并且每轮至少缩短 20% 和 5 秒、候选 p95 不高于 36 秒；失败即 no-go。普通资源安装事务、脚本、资源清单、Store、UI、PRoot View、AI 会话及 RF1500～RF1700 路径均不改。

## ADR-RF-082 固定真实包矩阵通过，允许最小生产调度样板

- 状态：已接受，RF1820 已完成，RF1830 进行中
- 日期：2026-08-01
- 决定：允许把 `DependencyBatchScheduler` 作为 RF1830 最小生产样板接到首次内置资源准备。调度器保持最大 2 槽、显式依赖、失败只阻断后继、独立任务继续和输入顺序归并；非法图继续在首个任务前整体 Blocked。生产接入只能替换当前串行循环，不能改变安装函数或每资源事实。
- 原因：固定 JVM 反例通过；OnePlus 真实 APK 包三轮结果零差异，串行 55,630～68,948ms 降到 13,704～20,412ms，每轮减少 68.9%～76.3%、至少 38,320ms，候选 p95 20,412ms。6 项全部成功、最大活动数 2、正式依赖 1 条，进程与临时夹具清理完成，Stage 266 tests 和 Debug 构建通过。
- 影响：RF1830 必须让 6 个正式任务从清单关系得到显式依赖并委托同一调度器；安装脚本、资源根、Store 注册、摘要顺序和启动进度所有权不变。固定矩阵需改为委托生产调度器，避免平行候选。父任务仍需 OnePlus 真实生产链和唯一 Full 才能放行；AI 会话继续冻结。

## ADR-RF-083 首次内置资源准备接入生产依赖调度

- 状态：已接受，RF1830 已完成，RF1840 进行中
- 日期：2026-08-01
- 决定：首次内置资源准备保留 6 项原安装函数，只把串行等待替换为最大 2 槽的显式依赖调度。系统工具依赖 Node；执行异常或前置失败的未执行项通过既有 `ToolchainResourcePort` 记录失败，无关项继续，所有结果按输入顺序归并。Debug 固定矩阵直接读取生产调度合同，不保留平行候选。
- 原因：生产接入后的 OnePlus 真包三轮零差异，串行 62,200～64,940ms 降为 18,303～21,008ms，每轮减少 67.7%～70.6%、至少 42,749ms，最大活动数 2；性能、正确性、进程和夹具清理门均通过。Targeted 16、Quick 266、Stage 278 tests 与 Debug 构建零失败，当前进程无 FATAL/ANR。
- 影响：`KiteResourceInstallStore`、`RuntimeBootstrapProgress`、安装脚本、参数、资源根和最终摘要所有权不变；页面没有参与调度，也没有新增刷新、轮询、重型绑定或第二 Store。RF1840 仍须关闭真实生产准备链和父任务唯一 Full，才能 production go；普通资源事务、PRoot View、AI 会话、魅族、main/其他工作树、远端、版本与发布继续冻结。

## ADR-RF-084 RF1800 首次内置工具链依赖调度生产放行

- 状态：已接受，RF1800/RF1840 已完成，production go
- 日期：2026-08-01
- 决定：保留首次内置 6 资源的显式依赖调度作为生产行为，最大活动数为 2，系统工具依赖 Node；无关任务可并行，失败只阻断后继，结果按输入顺序写回既有 Store。该结论不外推到普通资源事务、任意 shell 或任意卡片步骤。
- 原因：生产调度真包三轮与串行零差异，62,200～64,940ms 降为 18,303～21,008ms，每轮减少 67.7%～70.6%、至少 42,749ms；生产入口真实 6 资源/1 依赖复用登记链为 2,301ms 且失败数为 0。唯一 Full 1503 tests 零失败/零错误，OnePlus 当前进程无 FATAL/ANR。
- 影响：收益集中在首次安装、修复或资源缺失时的底层工具链整批等待；已经完整安装时仍走原文件校验和 Store 登记，不夸大为每次应用启动都减少 40 秒。下一父任务必须重新审计真实生产调用面，不能重复 RF1500～RF1800 矩阵；AI 会话和既有边界继续冻结。

## ADR-RF-085 RF1900 选择默认 Ubuntu 容器冷启动复用证明

- 状态：已接受，RF1910/RF1920 已完成，RF1930 进行中
- 日期：2026-08-01
- 决定：RF1900 只研究普通默认容器的进程冷态首次取得。当现有结构化候选完整证明运行时资产/描述符、基础镜像、默认容器记录、容器 rootfs 标记/必需文件和工作区目录 Ready 时，候选可进入固定矩阵；任一事实未知、不匹配或属于显式 View/环境、首次安装、修复、重置、Canary 时，整条走原完整准备。
- 原因：OnePlus 冷进程日志中默认容器完整准备已有 919ms 基础镜像阶段和 787ms 容器阶段，其中工作区支持/系统组件为 309/181ms；同一进程稍后的普通启动仍付 79ms 系统组件校准。相反，内置工具链已就绪复验仅 25～82ms，Supervisord 固定检查为 59ms且已有单飞，均不足以建立高收益父任务。
- 影响：RF1920 必须在生产改动前证明固定反例、候选零写入、同一容器身份和 OnePlus 三轮每轮至少 50%＋700ms、候选 p95 500ms、端到端 20%＋500ms。只有全部通过才允许最小接入；不新增 Store，不缓存动态网络/View/运行状态，不修改 PRoot 命令、页面或 AI 会话。

## ADR-RF-086 RF1900 冷进程复用矩阵放行

- 状态：已接受，RF1920 已完成，允许 RF1930 最小生产样板
- 日期：2026-08-01
- 决定：接受 `DefaultContainerColdReuseProvider` 作为默认容器进程冷态复用的结构化前置判断。Ready 必须同时证明当前 APK 的 PRoot 资产选择与已安装描述符一致，以及基础镜像、默认容器记录、rootfs 标记/必需文件、工作区和准备身份完整；显式 View/环境或任一未知事实继续走原完整准备。RF1930 只能在该边界内接入，并必须保留尚未被结构化事实覆盖的可变自修复。
- 原因：OnePlus 三轮独立冷进程候选为 214/205/210ms，原完整准备为 2078/2067/2061ms，每轮减少 1851～1864ms、89.7%～90.1%；候选 p95 214ms，三轮均无文件副作用且解析到同一容器身份。Stage 61 suites/285 tests 零失败/零错误，FATAL/ANR 为 0，超过所有预设正确性与收益门。
- 影响：RF1920 的证据只放行最小生产样板，不等于允许跳过目录权限、主机组、时区、包管理器目录、工作区组件等自修复语义。RF1930 仍须建立生产冷进程与反例证明；RF1940 才运行父任务唯一 Full 并决定 production go。

## ADR-RF-087 RF1900 以版本化物理收据接入冷进程复用

- 状态：已接受，RF1930 已完成，进入 RF1940
- 日期：2026-08-01
- 决定：普通默认容器跨进程复用必须持有最近一次完整修复写出的版本化物理收据。收据绑定 APK 安装代次、运行时文件、容器身份、Host 时区、rootfs 关键文件/权限、工作区静态组件和必要动态路径；Provider 每次冷进程只读复算，任一不一致在副作用前进入原完整准备。Ready 仍必须重新生成动态网络配置，不把网络、View 或运行状态写入收据。
- 原因：仅保留全部自修复的第一版三轮只减少 27.8%～30.0%/523～582ms，未达预设门；消除同一调用内重复校准并以 `Os.chmod` 取代每目录子进程后，再由收据关闭跨进程正确性，最终三轮从 1550/1535/1546ms 降到 479/475/469ms，减少 69.1%～69.7%、至少 1060ms，候选 p95 479ms。固定收据失配反例成功触发完整准备并恢复 Ready。
- 影响：APK 重装/更新、时区变化、关键文件或权限变化、工作区证明变化都会付一次完整修复，之后才恢复快路；这是有意的失败关闭。首次安装、修复、重置、Canary、显式 View/环境、动态网络、命令与页面语义不变。RF1940 必须完成唯一 Full、真实调用面和 FATAL/ANR 父门后才能 production go。

## ADR-RF-088 RF1900 默认 Ubuntu 容器冷进程复用生产放行

- 状态：已接受，RF1900/RF1940 已完成，production go
- 日期：2026-08-01
- 决定：保留版本化物理收据驱动的默认容器冷进程复用。Ready 只跳过已由收据证明等价的运行时/容器/工作区重复准备，并逐进程重建动态网络；收据未知或失配继续完整修复。结论只覆盖普通默认容器，不外推首次安装、更新、Canary、显式 View/环境或任意 PRoot 命令。
- 原因：OnePlus 最终三轮从 1550/1535/1546ms 降到 479/475/469ms，每轮减少 69.1%～69.7%、至少 1060ms，候选 p95 479ms；失配反例回完整修复并恢复 Ready。真实 App 冷启动进入 `MainActivity`，首屏 `TotalTime=1404ms`，正式后台日志命中收据快路。唯一 Full 291 suites/1514 tests 零失败/零错误，FATAL/ANR=0。
- 影响：新进程第一次取得已修复默认容器的固定等待约减少 1 秒；不宣称首屏整体减少 69%，也不改变 PRoot 兼容能力。下一父任务进入 RF2000 重新审计，不重复 RF1500～RF1900 矩阵；AI 会话和既有边界继续冻结。

## ADR-RF-089 RF2000 选择普通 PRoot 首份启动配置复用

- 状态：已接受为研究候选；RF2020 后由 ADR-RF-090 production no-go 收口
- 日期：2026-08-01
- 决定：RF2000 只研究普通默认容器 Ready 后的冷进程首份 PRoot 启动配置。Ready 可以复用 RF1900 收据已经覆盖的 runtime layout、默认 container 和 workspace 静态事实；Android 共享存储、动态网络、View/环境解析、Node 缓存、telemetry、运行状态和最终命令仍逐次构造。任一收据/身份事实未知或失配、显式 View/环境、首次安装/修复/重置和 Canary 都在业务进程前走现有完整准备。
- 原因：四个配置构造 API 被终端、卡片/资源、后台、Agent 与运行时管理共同使用，而现有进程缓存只能复用第二份配置；首份仍重复已由收据证明的 `prepareRuntime`、容器解析和工作区静态组件校准。跨层准备令牌需要扩大公开协议但覆盖相同，原生 tar/xz 尚无封闭权限/链接合同且既有 ZIP 候选比 PRoot 慢约 202.8%，均不是当前第一名。
- 影响：RF2020 必须用不可由 ADB 改样例的固定冷进程矩阵证明 command/env/容器与全部投影等价、零业务进程、固定反例回完整路径，以及 OnePlus 三轮每轮至少 30%＋300ms、候选 p95 不高于 500ms。全部通过前不得接生产；不新增 Store，不修改 UI、PRoot 命令、AI 会话或其他冻结边界。

## ADR-RF-090 RF2000 普通 PRoot 首份启动配置生产 no-go

- 状态：已接受，RF2000/RF2020/RF2040 已完成，RF2030 未触发
- 日期：2026-08-01
- 决定：不把 RF2020 的静态准备候选接入生产。保留固定 Debug 矩阵、internal benchmark helper 和等价的 argv 配置组装提取；正式 `prepareOrdinaryBuildContext` 继续走现有完整准备。以后只有候选在相同固定反例下达到 p95 不高于 500ms，才允许重开生产接入。
- 原因：保守配对基线为 1298/1294/1294ms，候选为 889/878/890ms，每轮减少 31.2%～32.1% 与 404～416ms，身份和完整 command/env digest 一致且零业务进程；但候选 p95=890ms，未达到生产改动前锁定的 500ms 上限。显式 View/环境与收据失配反例均正确失败关闭。Quick 269、Stage 621、唯一 Full 1515 tests 零失败，Debug 构建与 OnePlus FATAL/ANR 门通过。
- 影响：Kite 用户当前不会获得这 0.4 秒候选收益，也不会承担一个仍需约 0.89 秒且未达标准的快路。终端、卡片/资源、后台、Agent、PRoot View、Store 与 AI 会话行为保持不变；本轮按用户合并窗口收尾，不启动 RF2100。
