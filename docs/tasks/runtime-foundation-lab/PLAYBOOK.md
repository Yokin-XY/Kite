...
- [ ] OnePlus 8T 真实已安装资源批量检查至少包含 2 个目标，记录分车道并发摘要且每目标只执行一次；
- [ ] 用户点击仍先得到全部“检查中”状态，结果只局部更新，现有汇总文案和顺序不变；
- [ ] Targeted、Quick、Stage、Debug 构建、FATAL/ANR 和生产范围审查通过；
- [ ] RF1700 父门唯一一次 Full 零失败后才可 production go；
- [ ] AI 会话、安装/更新事务、单项检查、PRoot View、魅族、main/其他工作树、远端、版本和发布保持不动。
