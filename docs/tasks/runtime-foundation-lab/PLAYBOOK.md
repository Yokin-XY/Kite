...
### RF1700 [P1 业务通用能力] 批量版本检查分车道有界并发

父任务目标：缩短资源管理页“检查已获取资源更新”的整批等待，不改变单项检查、版本语义、Store 所有权或页面结构。批量入口必须先为全部目标写入既有“检查中”状态，再按结构化版本计划把可由 Android 原生元数据＋远端请求完整表达的项目放入最多 3 个并发槽；需要显式命令/PRoot 的兼容项目最多 1 个。所有结果仍按输入顺序应用，单项失败不改变其他项目事实，取消不遗留工作。AI 会话协议、消息、流式输出、会话恢复、附件和样式全部冻结。

#### RF1710 候选与发布门审计

- 问题证据：`AndroidResourceActionGateway.checkUpdates()` 已在首个探针前批量写入乐观状态，但随后用普通 `targets.map` 串行执行全部 `versionCoordinator.check()`；5 个默认 npm 正式资源已具备原生本地版本与独立远端版本请求，彼此没有依赖，却仍累计网络等待；
- 候选排序：第一名为批量版本检查分车道有界并发；第二名显式 `--version` 结构化迁移因 CLI/包版本语义可能不同而 no-go；第三名 Ubuntu `/run`/locale 启动探针因 rootfs 权限和启动语义证据不足而 no-go；
- 解法：先建立入口无关的固定五请求调度矩阵，冻结顺序、并发上限、取消、失败隔离与性能门；RF1720 前不修改生产批量 Gateway；
- 验收标准：正式批量入口和至少 5 个正式资源复用；不含资源/包/页面特判；AI 会话范围静态冻结；独立文档提交；
- 依赖：RF1600 已以真实双车道链与 Full 1487 tests 完成，RF1550 的分层测试和 Gradle 锁可继续使用。

#### RF1720 固定调度矩阵与基线

- 固定 5 个独立请求，3 个结构化原生/远端、2 个兼容请求；样例、延迟、轮数、并发上限和阈值不能由 ADB 或测试参数覆盖；
- 正确性门：三轮结果与串行基线零差异，输入/输出顺序不变；首个探针前全部进入检查中；原生/远端最大并发不超过 3，兼容最大并发不超过 1；单项失败不抹掉其他结果，取消后无活动任务；
- 性能门：固定五请求批次三轮 p50 每轮相对串行至少降低 40%，候选 p95 不高于 550ms；任一门失败即 no-go，不写生产样板；
- 只测本轮批量调度，不重复 RF1500/RF1600、Node/Python 或任意 shell 矩阵。
- [x] OnePlus 固定矩阵三轮零差异，顺序、检查中先写、失败隔离、取消清理和 `3/1` 并发上限全部通过；
- [x] 串行 913/911/910ms，Debug 候选 380/370/372ms，每轮减少 58.4%/59.4%/59.1%，候选 p95 380ms；
- [x] Targeted、Quick 261 tests、Stage 264 tests、Debug 构建和 FATAL/ANR 检查通过，允许进入 RF1730。

#### RF1730 最小生产调度样板

- 只有 RF1720 全部门通过才开始；调度器只消费结构化 lane 分类和挂起任务，不读取资源 ID、包名、页面或命令文本；
...
- [x] 固定五请求矩阵、顺序、失败、取消和并发上限全部通过；
- [x] OnePlus 8T 真实已安装资源批量检查包含 2 个正式目标，生产摘要为原生/远端 1、兼容 1、目标去重 2、检查中残留 0；
- [x] 全部“检查中”仍先于预检和首个业务进程写入，结果继续按输入顺序交给既有 Store 与单个汇总 Effect；页面与局部更新链未改；
- [x] Targeted、Quick 262 tests、Stage 282 tests、Debug 构建、FATAL/ANR 和生产范围审查通过；
- [x] RF1700 父门唯一一次 Full 为 288 suites、1497 tests、零失败、3 个既有平台跳过，production go；
- [x] AI 会话、安装/更新事务、单项检查、PRoot View、魅族、main/其他工作树、远端、版本和发布保持不动。

### RF1800 [P1 业务通用能力] 内置底层资源依赖调度

父任务目标：缩短首次准备 Kite 底层环境时 6 个正式内置资源的整批等待，不改变任何资源的安装脚本、Ubuntu/PRoot 语义、登记所有权或页面结构。调度必须消费显式依赖图，在首个安装进程前确定可运行集合；最多同时运行 2 项，依赖失败的后继不启动，独立任务继续，最终结果与摘要按输入顺序归并。AI 会话协议、消息模型、流式输出、会话恢复、附件、显示样式、模型和权限 UI 全部冻结。

#### RF1810 候选与发布门审计

- 问题证据：`ToolchainPackInstaller.runPreparePackLocked()` 对 Node.js、Python、uv、Git、curl、系统工具 6 个正式资源使用 `mapIndexed` 完全串行；OnePlus 8T 现有真实成功记录从 `startedAt=1785472682584` 到 `updatedAt=1785472726995`，整批为 44,411ms；其中系统工具显式依赖 Node.js，其余任务没有资源依赖，且各自写入独立资源根；
- 候选排序：第一名为内置底层资源依赖图＋最多 2 槽有界调度；第二名“获取前资源清单强制重载”只重读 17 份、68,450 字节清单，预期为细节收益；第三名“连续 PRoot 步骤复用”在正式资源安装中已被事务编译器合并为单个 shell 步骤，剩余主要是用户自建多步卡片，当前缺少多个正式资源覆盖；
- 解法：RF1820 先建立不可由 ADB 改写任务、依赖、时长、槽位或阈值的固定调度矩阵，并用真实内置包负载建立串行/候选基线；门通过前不修改生产 `runPreparePackLocked()`；
- 验收标准：固定 6 任务结果、输入顺序、依赖阻断、失败隔离、最大活动数和清理零差异；候选最大活动数不超过 2，依赖任务不能先于前置成功启动；OnePlus 真实包三轮每轮相对串行至少缩短 20% 且至少 5,000ms，候选 p95 不高于 36,000ms；任一门失败即 no-go；
- 依赖：RF1700 已以真实双车道批量链和 Full 1497 tests 完成；RF1550 的 Targeted/Quick/Stage/Full 与 Gradle 串行包装器继续适用。

#### RF1820 固定依赖矩阵与真实包基线

- 固定 6 任务与正式依赖形状一致：5 个无前置任务，1 个任务依赖其中 1 个；同时固定成功、独立失败、依赖阻断、结果顺序和并发上限反例；
- 固定真实包负载必须使用 APK 内置 Node/Python/uv/pnpm 包与受控临时根，串行和候选读取同一只读包，输出写入彼此隔离的临时目录，结束后全部清理；ADB 只能触发，不能传资源、包、依赖、路径、轮数、槽位或阈值；
- 正确性门：三轮零差异，结果按输入顺序；最大活动数 `<=2`；前置未成功时后继绝不启动；单项失败不取消无关任务；任务、进程和临时目录无残留；
- 性能门：三轮每轮相对串行至少缩短 20% 且至少 5,000ms，候选 p95 不高于 36,000ms；门失败即 no-go，不写生产样板；
- 只测本轮首次底层资源整批准备，不重跑 RF1500～RF1700、Node/Python Host、任意 shell 兼容或 AI 会话矩阵。
- [x] 通用调度合同覆盖输入顺序、2 槽、依赖成功后释放、失败隔离、异常隔离，以及重复键/缺失依赖/依赖环零副作用阻断；
- [x] OnePlus 固定真实包三轮结果零差异，6 项全部成功、正式 1 条依赖、最大活动数 2、进程和夹具清理通过；
- [x] 三轮串行 57,809/68,948/55,630ms，候选 13,704/20,412/17,310ms，每轮减少 68.9%～76.3%、至少 38,320ms，候选 p95 20,412ms；
- [x] Stage 57 suites/266 tests、Debug 构建与 FATAL/ANR 通过，允许进入 RF1830；生产 `runPreparePackLocked()` 仍为串行。

#### RF1830 最小生产调度样板

- 只有 RF1820 全部门通过才开始；通用调度器只读取任务键、显式依赖集合和挂起工作，不读取资源 ID、资源名、命令、页面或包身份；
...
- [x] 建立纯判断 Provider 与固定反例；Ready 需要当前 APK 内的 PRoot 选择与已安装描述符一致，并同时核对基础镜像、默认容器记录、rootfs 标记/必需文件、工作区和身份；显式 View/环境与任一未知事实均失败关闭；
- [x] Debug 接收器不接收 ADB 参数，只读正式事实并对候选前后文件戳取样。OnePlus 三轮独立冷进程候选为 214/205/210ms，原完整准备为 2078/2067/2061ms，每轮减少 1851～1864ms、89.7%～90.1%，候选 p50 210ms、p95 214ms；三轮 `noSideEffects=true`、`sameIdentity=true`；
- [x] 定向测试、Stage 61 suites/285 tests、Debug 构建、三轮固定矩阵和 FATAL/ANR=0 全部通过，超过每轮 50%＋700ms、候选 p95 500ms、端到端 20%＋500ms 的预设门，允许 RF1930 最小生产接入。

#### RF1930 最小生产复用样板

- [x] 第一版只跳过基础镜像、仍完整执行容器可变修复，三轮正式路径 1335～1361ms、只减少 523～582ms/27.8%～30.0%，未达预设门；没有降低门槛，而是继续消除同一次准备中重复的容器/工作区校准，并用 Android `Os.chmod` 替换每目录一次 `/system/bin/chmod` 子进程；
- [x] 完整修复结束后写入版本化物理收据；收据绑定 APK 安装代次、运行时文件、容器/时区、rootfs 关键文件与权限、工作区静态组件及必要动态路径。Ready 冷进程只复算收据并重建动态网络；安装代次或任一证明变化均在副作用前回完整准备；
- [x] 固定失配反例得到 `fallbackTriggered=true/repaired=true`。OnePlus 三轮配对完整准备 1550/1535/1546ms，正式快路 479/475/469ms，分别减少 1071/1060/1077ms、69.1%/69.1%/69.7%，候选 p95 479ms；每轮身份一致且基础镜像不变；
- [x] 不新增 Store，不缓存动态网络/View/运行状态，不修改页面或普通 PRoot 启动语义；Targeted、Quick 58 suites/268 tests、Stage 62 suites/291 tests（1 个既有平台跳过）、Debug 构建与范围审查通过。

#### RF1940 go/no-go 与父任务门

- [x] OnePlus 真实 `StartupGuardActivity` 冷启动进入 `MainActivity`，系统首屏 `TotalTime=1404ms`；正式后台准备日志命中 `mutableRepair=receipt_verified`。固定收据失配反例回完整修复并恢复 Ready；当前容器/工作区身份一致，FATAL/ANR=0；
...
#### RF2010 真实生产调用面与收益门审计

- [x] 第一名为普通 PRoot 首份启动配置复用：四个配置构造 API 被终端、卡片/资源、后台、Agent 与运行时管理共同调用；现有进程内缓存只覆盖同进程第二份配置，首份仍重复 `prepareRuntime`、容器解析和工作区静态组件校准；
- [x] 第二名“ensure 到配置的跨层准备令牌”覆盖面相同但需要扩大公开交接协议、协调令牌生命周期与失效，正确性风险高于在 `KFContainerManager` 内复用既有身份；第三名原生 tar/xz 尚无封闭权限/链接合同，且既有 ZIP 候选比 PRoot 慢约 202.8%，均不进入本轮生产样板；
- [x] 状态仍归容器注册表与 `RuntimeLaunchPreparationCache`；不新增 Store，不缓存动态网络/View/运行状态，不修改页面或调用方业务协议。AI 会话全范围继续冻结。
