...
- [ ] 新增结构化原生能力步骤，不从 `curl` 或资源 ID 推断；
- [ ] 同一 `CardRun` 写入 `android_native` 车道、进度、结果与取消事实；
- [ ] 页面只消费运行状态，不创建终端或整页刷新。

##### RF310c 资源获取迁移与压力门

- [ ] 资源下载步骤显式编译为原生能力，其他安装步骤仍保留 PRoot 兼容；
- [ ] 固定 PRoot 对照、网络中断/重试、空间不足和大文件压力；
- [ ] 不改变更新锁、备份、验证和安装状态拥有者。
