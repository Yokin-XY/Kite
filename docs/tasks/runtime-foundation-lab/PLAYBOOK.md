...
- 输出长期上限、长期占用和余量保护的固定低基数字段；
- Debug 固定矩阵覆盖 1/2/4、后到短任务、恢复超额与压力收缩；
- OnePlus 8T 验证不增加业务进程、不产生 FATAL/ANR。

#### RF1040 父任务门

- 强制全量单测与 Debug 构建；
- 复核 Node/Python 无性能矩阵重跑，终端/Agent 无净迁移；
- 独立提交并给出下一类别是否值得迁移的 go/no-go。
