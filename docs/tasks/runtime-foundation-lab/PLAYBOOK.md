...
- [ ] 仅在 RF1920 全部门通过后，让 `ensureDefaultContainer()` 的普通默认候选 Ready 分支建立进程内身份并返回既有容器；对目录权限、主机组、时区、包管理器目录和工作区组件等可变自修复事实，必须先证明已被结构化事实覆盖，或继续执行轻量修复；其余分支原样进入完整准备；
