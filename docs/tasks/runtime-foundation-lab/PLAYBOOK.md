...
父任务目标：从 RF1900 完成后的真实生产调用面与兼容债务重新筛选最多三个候选；先固定跨调用方覆盖、正确性反例和端到端收益门。连续两轮无候选达门或只剩单页面/低频/几十毫秒项目时进入细节优化区间，不为延续任务降低门槛。

#### RF2010 真实生产调用面与收益门审计

- [ ] 重新统计正式调用方、固定成本、状态拥有者和既有单飞/缓存，排除 RF1500～RF1900 已完成矩阵；
- [ ] 最多三个候选按覆盖面、正确性风险和可感知收益排序；第一名不达预设门则 no-go，不写生产补丁；
- [ ] AI 会话全范围继续冻结；不触碰 UI、魅族、main/其他工作树、远端、版本或发布。
