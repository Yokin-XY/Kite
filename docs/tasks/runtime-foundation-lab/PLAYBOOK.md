...
- 状态：进行中。
- 使用固定结构化命令，对比不协调、只包围 `start()`、包围到首个 READY 三种模式；
- 覆盖 1/2/4/8 同时请求及窗口 1/2/4，输出 batch wall、ready P50/P95、失败率与残留；
- 矩阵不接收外部命令、路径、并发或业务身份，构建物不进入 Git。
