...
- [ ] 同一活动或排队 jobId 不能重复取得准入；
- [ ] 排队任务可按稳定 jobId 取消，释放等待和写屏障且不杀死已开始任务；
- [ ] snapshot 可区分活动、排队、取消和超时，不接管业务结果。

##### RF420c 策略与父任务门

- [ ] 交互优先、同优先级 FIFO、共享写防饥饿、压力收缩和关闭行为通过回归；
- [ ] 固定维护任务的 warm/独立回退继续共用同一 lease，Debug 构建通过；
- [ ] 形成下一阶段可接入的有界短任务门，不按命令名或资源 ID 路由。
