...
活动目录枚举成功后，某个带强身份的旧会话整体不再存在时，`ProotTelemetryStore` 应把该会话仍标记为运行中的历史记录统一收敛为退出；刚创建的会话保留短暂发布宽限。目标会话自身出现在 unstable 集合时不做退出判断，但无关会话正在更新或损坏不能阻塞已经消失的目标会话。文件事件必须先重放，活动注册表再做最终收敛。这样历史 JSONL 只承担审计和恢复，不会把已经消失的整批进程重新投影成“未归属”。

后台运行项与卡片、终端使用同一套 `KF_RUNTIME_ID` / `KF_ROOT_RUNTIME_ID` / `KF_UNIT_ID` 身份环境。身份必须在 `BackgroundRuntimeHost` 启动进程时一次性注入，子进程再沿 PRoot 父生命周期继承；运行管理不得根据 `sleep`、`bash` 等进程名称猜测后台基础设施。

同一 PID 同时出现在通用运行根观察和 PRoot 生命周期表时，带强生命周期身份的 PRoot 记录优先于 `UNATTRIBUTED`（未归属）运行根；真实卡片、终端、资源和后台 owner 根仍保留为上层控制入口。弱观察记录不得遮蔽强身份并把可控子进程降级为兼容只读行。
...
作用域身份始终保留在数据层；可见成员只有一个时，移动端直接显示标准进程行，不重复套一层“1 个进程”组头。成员达到两个及以上时才显示可展开组。这只是展示密度规则，不改变作用域结束和后续成员汇入语义。
