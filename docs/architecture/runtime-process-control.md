...
Kite 的卡片不是 Linux namespace 或 Docker 容器，而是一个稳定的生命周期 owner：由该卡片命令产生的进程都继承 owner。运行管理、单进程结束、应用树结束和卡片停止必须消费同一份 owner 事实，不能分别扫描和猜测。
...
Android 全量恢复可以读取所有可见会话；停止某个 owner 时只调用 `readSessions(sessionIds)` 读取该 owner 已登记的会话。无关会话正在更新不能阻塞当前停止，目标会话目录消失表示稳定空集，隐藏退役目录不会重新导入旧事实。
...
使用同一快照中的 `parentLifecycleSeq`/PPID 建树，子进程优先于父进程结束。每个成员仍逐一通过强身份核验；缺少强身份的组不展示整组结束能力。
...
最后一条尤其重要：PRoot 启动的多个卡片以及 Kite 可能共享 Android 进程组。PGID 适合展示 Linux 拓扑，但没有独占证据时不是卡片隔离边界。历史上的进程组兜底会同时结束 A、B 和 Kite，已经从正常路径和 Bridge 二次兜底中删除。
...
1. owner 仍是产品生命周期身份；
2. PRoot 创建进程时把成员加入 owner 对应 cgroup；
3. 运行管理仍从统一事实快照投影，不从 cgroup 另建页面状态；
4. `CgroupBackend` 可以替代 owner 成员枚举和批量发送，但执行前后仍校验 owner/cgroup 对应关系并确认空集；
