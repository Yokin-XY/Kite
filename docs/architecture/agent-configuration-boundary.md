...
| 来源 | 准确定义 |
| --- | --- |
| 免费 | 无需登录、无需 URL、无需 API Key，选择后直接可用；首建于 OpenCode，现覆盖六 Agent 的免费分组 |
...
供应商配对预置采用双源（2026-09 起）：内置 CC Switch 转换的全量预设目录（`CcSwitchCatalogBundle`，约 590 条，随 APK、无网可用，只作首次快照与免费候选）；在线时用户打开配对页可从 `models.dev` 拉取供应商、端点和
模型事实。模型目录不按 `models.dev` 单条记录的路由协议过滤；配置层只按目标 Adapter 的真实协议选择
可写入的请求路线，并把同厂商、同市场、同渠道的最新模型目录合并到该路线。页面必须分别显示模型目录
来源和请求路线来源。预置再按官方、国内官方、云平台、聚合平台、第三方及
API、Coding Plan、Token Plan 等访问渠道组织。失败时依次使用上次成功缓存和随应用预置；页面始终允许
手动填写。同一厂商的国内、国际和套餐入口共用稳定 `vendorId`，完整渠道名、厂商品牌、市场和访问渠道
分别保存；没有明确来源时市场保持未声明，页面不得从公司国别臆测。选中预置只是填充可编辑表单，保存后
仍作为用户自定义 Provider 进入统一持久目录。
