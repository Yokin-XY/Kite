...
| 编号 | 当前未放行能力 | 已确认原因 | 当前兼容路线 | 未来候选，不代表承诺 |
| --- | --- | --- | --- | --- |
| NATIVE-ARCHIVE-01 | 将资源 ZIP 默认迁到 Android 安全解包 | Rust Debug 引擎已证明性能、安全反例、摘要拒绝和原子清理；尚无正式实时取消/进度桥、多设备门和资源事务接入 | 资源安装继续现有 PRoot/事务链；原生 ZIP 只供显式受控调用 | 保持粗粒度 JNI，关闭生产接线门后按制品类型逐项放行 |
| NATIVE-ARCHIVE-02 | 将 `tar.gz`/`tar.xz` 默认迁到 Android 原生事务 | 魅族 18 四份真实制品已证明 Rust 约 2.5～3.7 倍于 Kotlin/PRoot 且完整树一致；rootfs 设备节点和硬链接必须继续采用现行显式降级语义 | 正式安装仍走现有 Kotlin/PRoot 与资源事务 | Rust 只持有校验、解包和原子目录发布；锁、状态、回滚、进度和取消继续由现有 Kotlin owner 接入 |
