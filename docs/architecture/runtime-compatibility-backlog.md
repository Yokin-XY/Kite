...
| GUEST-SYSCALL-02 | openat2(437) 被 KILL（fs-safe 等 Rust 内联直发） | rustix/fs-safe **无** ENOSYS 降级（实测 -38 被当 fd 用） | **已落地并固化（ce408b4e）**：调用点现场翻译为 openat，manifest 声明式 binaryPatches 机制随会话启动自动应用（幂等、计数校验、失败关闭）；openclaw 重装免疫 | 同上 |
