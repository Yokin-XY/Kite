...
internal class AndroidManagedAgentRuntimeDependencyPreparer(context: Context) :
    ...
    /** 机器令牌类环境文件（random_token）缺失时自动补齐；幂等，存在即跳过。 */
    private fun ensureEnvironmentFilesPresent(
        specs: Map<String, com.kite.app.resources.KiteResourceEnvironmentFileSpec>,
    ) {
        runCatching {
            com.kite.app.resources.KiteResourceEnvironmentFileStore.ensureAutoGenerated(
                hostFileFor = { containerPath ->
                    ContainerVisibleFileResolver.resolve(appContext, containerPath)
                },
                specs = specs,
            )
        }
    ...
    override suspend fun prepare(dependencies: List<KiteResourceAgentRuntimeDependency>) {
        ...
        dependencies.forEach { dependency ->
            ...
            val definition = BackgroundRuntimeRecord(
                ...
                environmentFiles = dependency.environmentFiles.mapValues { it.value.path },
...
internal class AndroidAgentRecipeRuntime(
    ...
    private fun ensureEnvironmentFilesPresent(
        ...
        runCatching {
            com.kite.app.resources.KiteResourceEnvironmentFileStore.ensureAutoGenerated(
                hostFileFor = { containerPath ->
                    com.kite.app.foundation.workspace.ContainerVisibleFileResolver.resolve(appContext, containerPath)
                },
                specs = specs,
            )
        }
