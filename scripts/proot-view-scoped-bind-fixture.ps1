...
$remoteScript = '/data/local/tmp/kite-proot-view-scoped-bind-fixture.sh'
$remoteCandidate = '/data/local/tmp/kite-proot-view-candidate'
$privateCandidate = 'files/runtime/bin/proot-view-benchmark'
$localScript = Join-Path $PSScriptRoot 'proot-view-scoped-bind-fixture.sh'
...
if ($LASTEXITCODE -ne 0) { throw '推送作用域 View 夹具失败。' }
& $adb -s $DeviceSerial shell chmod 755 $remoteScript
if ($LASTEXITCODE -ne 0) { throw '设置作用域 View 夹具权限失败。' }
...
$outputDirectory = Join-Path $repoRoot "local-artifacts\proot-view-scoped-bind\$timestamp"
...
if ($exitCode -ne 0) {
    throw "作用域 View 夹具失败，退出码 $exitCode；输出：$resultPath"
}
if (-not ($lines -contains "result`tSCOPED_BIND_FIXTURE_OK")) {
    throw "作用域 View 夹具缺少成功标记；输出：$resultPath"
}
