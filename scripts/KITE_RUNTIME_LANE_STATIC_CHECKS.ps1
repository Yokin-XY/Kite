...
Assert-True ($startRecipe -match 'ownerKind = previousState\.ownerKind' -and $startRecipe -match 'stepId = previousState\.stepId') 'startRecipe must preserve the existing CardRun owner identity when a prepared resource/terminal state starts.'
...
Assert-True ($containerProcessStore -match '(?s)else if \(force\).*isContainerProcessAlive\(context, pid\).*killUbuntuProcessPid\(.*HostProcessTerminator\.killHostProcess') 'manual force termination must kill live Ubuntu/container PIDs before falling back to host PID kill.'
...
Assert-True ($prootOwnerTerminator -match 'Os\.kill\(target, signal\)' -and $prootOwnerTerminator -match 'signalTraceePidsLazily' -and $prootOwnerTerminator -match 'SIGNAL_BATCH_DELAY_MS' -and $prootOwnerTerminator -match 'OWNER_STOP_DEADLINE_MS = 10_000L') 'owner stop must signal bounded PRoot tracee pids lazily through Android platform signals within a 10s budget.'
Assert-True ($prootOwnerTerminator -match 'waitForOwnerExit' -and $prootOwnerTerminator -match 'KILL_WINDOW_MS') 'owner stop must wait lazily after TERM and reserve a final KILL window.'
...
Assert-True ($prootOwnerTerminator -notmatch 'sendSignal\(-' -and $prootOwnerTerminator -notmatch 'Os\.kill\(-') 'owner stop must not use negative process-group signals that can kill the app process group.'
...
Assert-True ($handleStopResultV2 -match 'stopLooksLikeManualKill\(result\)' -and $handleStopResultV2 -match 'stop_killed_output_suppressed') 'manual stop must suppress shell Killed output instead of flashing raw failure toast.'
