...
$legacyStopRecipeByCardInstanceId = Function-Body $main 'legacyStopRecipeByCardInstanceId'
...
Assert-True ($stopRecipeByCardInstanceId -match 'recipeUsesProcessRunOrchestrator' -and $stopRecipeByCardInstanceId -match 'stopRecipeWithOrchestrator' -and $stopRecipeByCardInstanceId -match 'legacyStopRecipeByCardInstanceId') 'Stop intake must explicitly separate migrated orchestration from the bounded resource compatibility path.'
...
Assert-True ($legacyStopRecipeByCardInstanceId -match 'hasProcessBindingForStop\(\)') 'Temporary resource compatibility stop must still distinguish terminal-only state from process bindings.'
Assert-True ($legacyStopRecipeByCardInstanceId -match 'stop_terminal_and_process_request_sent') 'Temporary resource compatibility stop must record the combined terminal/process request.'
Assert-True ($legacyStopRecipeByCardInstanceId -match 'bridgeClient\.stopProcessBinding') 'Temporary resource compatibility stop must ask the bridge to stop retained process bindings.'
...
Assert-True ($handleBridgeResult -match 'bridgeFailureArrivedAfterManualStop\(recipe, result\)' -and $handleBridgeResult -match 'bridge_killed_output_suppressed_after_stop') 'late bridge Killed callbacks after manual stop must be folded into Stopped, not surfaced as failures.'
