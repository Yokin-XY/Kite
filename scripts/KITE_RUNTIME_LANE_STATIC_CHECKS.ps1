...
Assert-True ($runtimeHealthStore -match 'val attributedOwnerIds = ownerRoots\s*\r?\n\s*\.mapNotNull \{ it\.ownerId \}\s*\r?\n\s*\.toSet\(\)' -and $runtimeHealthStore -match '\.filter \{ group -> group\.ownerId in attributedOwnerIds \}') 'RuntimeHealth must not hide unknown owner prefixes from unattributed roots.'
