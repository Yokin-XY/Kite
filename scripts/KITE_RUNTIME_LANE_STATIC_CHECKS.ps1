...
Assert-True (
    $cardRunActivity -match '(?s)override fun handleOnBackPressed\(\)\s*\{\s*if \(chrome\?\.handleBack\(\) == true\) return\s*if \(surfaceHost\?\.handleBack\(\) == true\) return\s*navigateBackFromTask\(\)\s*\}' -and
    $cardRunActivity -match '(?s)private fun navigateBackFromTask\(\).*CardRunTaskNavigationPolicy\.decide\(currentState\).*CardRunTaskNavigationAction\.HideTask -> moveTaskToBack\(true\).*CardRunTaskNavigationAction\.CloseTask -> closeTaskWindow\(CardRunTaskCloseReason\.DismissSurface\)' -and
    $cardRunActivity -match 'onExit = ::navigateBackFromTask'
) 'CardRun navigation must delegate nested history first, hide install-wizard tasks, and keep title-bar back on the same navigation path.'
