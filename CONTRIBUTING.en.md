...
1. Before an external contribution, material product-behavior change, or sensitive change, open or claim a public Issue. Existing community tasks can be claimed without a duplicate, and work originating in a Discussion should link back to it. Maintainer-only experiments, temporary performance diagnostics, build measurements, and low-risk fixes that do not change product behavior stay in local task tracking instead of creating public Issues for internal process.
2. Work that requires a public Issue must state the real use case, expected outcome, implementation mechanism, scope, risk, and verification plan. Update the Issue before any material mechanism change so code review can compare the implementation with the disclosed approach.
...
4. Each Pull Request must address one explainable behavior boundary. Work that requires a public Issue must link it; a low-risk maintainer fix that does not require one may use `N/A` in the template and explain why. Formal direction, provenance, dependency, permission, execution-surface, code, and verification review happens at the Pull Request stage. Automated checks do not replace human review.
