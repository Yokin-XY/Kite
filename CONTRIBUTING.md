...
1. 先通过 Issue、Pull Request 草稿或其他公开讨论确认一个边界清楚的方向。
2. 贡献者在自己的 Fork 和分支中实现，每个 Pull Request 只处理一个可解释的问题。
3. 项目方检查来源、代码、依赖、权限、资源执行面和验证证据；自动检查通过不代替人工审查。
4. 维护方根据审查结果决定是否合并；正式构建、签名和发布由明确承担对应职责的维护者执行。
5. 社区角色根据持续贡献质量、协作稳定性、安全意识和实际职责逐步确认，不以单次提交或单一数量指标自动升级。

社区角色分为：

- `Contributor`：通过 Issue、Fork 和 Pull Request 提交贡献；不需要主仓库写权限。
- `Collaborator`：经过持续贡献后，可以在主仓库创建工作分支，但仍通过 Pull Request 进入主线。
- `Maintainer`：在明确负责的范围内参与方向确认、审查和合并。
- `Release Maintainer`：负责正式构建、签名、制品校验和发布。

角色对应的是协作职责，不改变代码和项目依据许可证、贡献者协议及其他书面约定形成的权利边界。角色可以随着实际职责和安全需要调整。
